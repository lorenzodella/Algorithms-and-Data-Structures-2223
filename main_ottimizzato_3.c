...
Nodo* pianifica_percorso_indietro(Tree *autostrada, Nodo* start, int end){
    ...
    while(!pila_is_empty(&p)){
        ...
        if(p2.top!=NULL)
            prec = bst_predecessor(autostrada, p2.top->val);
        else
            prec = bst_predecessor(autostrada, n);
        autonomia = heap_getmax(n->stazione.parco_auto);
        if(autonomia<0)
            return NULL;
        while(prec!=autostrada->nil && n->stazione.distanza-autonomia <= prec->stazione.distanza){
            ...
            prec = bst_predecessor(autostrada, prec);
        }
        n->color=2;
        if(pila_is_empty(&p)){
            p.top = p2.top;
...
int pianifica_percorso(Tree autostrada){
    ...
    if(start==end){
        ...
        reset(&autostrada, autostrada.root);
        if( (n=pianifica_percorso_indietro(&autostrada, bst_search(&autostrada, autostrada.root, start), end)) != NULL){
            post_backtrace(n);
            printf("\n");
            return 1;
        }
